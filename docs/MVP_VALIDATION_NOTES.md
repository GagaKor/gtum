...
- agent-only decision coverage verifies command decisions and decision recording stay in the right panel without terminal runtime execution, that pending approvals disappear after a decision, and that the composer shows project scope instead of a current-tab context chip.
