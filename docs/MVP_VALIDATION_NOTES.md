...
## 2026-07-15 Claude Account Model Catalog And Picker Repair
...
- An explicit Claude request refreshes the catalog after authentication and before inference spawn. Only exact membership in returned `value` fields is accepted; `resolvedModel` is not an alternate selector. A valid value becomes one separate `--model <value>` pair, while `model: null` skips the extra probe and model flag.
...
- Picker acceptance requires compact non-wrapping provider/model/reasoning/fast triggers, exact names and current state inside opened lists, exact accessible trigger names, right-panel and viewport containment at the default desktop size, a single unclipped human-label line with no raw-ID subtitle, internal scrolling, selected-bottom-row visibility after reopen, one selected option, provider/session persistence, and zero center-terminal calls.
- Final gate evidence is pending: record the current focused parser/request/UI counts, complete lint/build/serial Playwright/Rust counts, production-path no-prompt compatibility smoke, Windows target status, independent review result, and non-force `dev` integration only after those checks finish.
