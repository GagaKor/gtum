...
- Windows cross-target compilation remains an environment validation caveat: Tauri's `tauri-winres` build script stops before GTUM crate compilation because this macOS host has no `llvm-rc`. Source and portable fake-child review found no Windows defect, but native Windows CI/installed-app validation is still required. Non-force `dev` integration is the only remaining step for this slice.
