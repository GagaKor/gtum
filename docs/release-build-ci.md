...
Tauri automatically merges platform-specific config files at build time. The shared app settings remain in `src-tauri/tauri.conf.json`; Windows bundle targets are pinned in `src-tauri/tauri.windows.conf.json`, and macOS bundle targets are pinned in `src-tauri/tauri.macos.conf.json`.
...
Windows and macOS release artifacts should be built on native OS runners. Do not depend on Windows-to-macOS or macOS-to-Windows cross-compilation for release acceptance; use per-platform configs and runner-native Tauri builds instead.

## Release Artifacts

The base `src-tauri/tauri.conf.json` keeps shared app and bundle settings. Platform-specific config files narrow the expected targets for each native build:
...
- `Windows`
  - `exe` or `msi`
  - configured through `src-tauri/tauri.windows.conf.json` as `nsis` and `msi`
  - generated by `npm run tauri:bundle` locally on Windows or by the Release workflow on `windows-latest`
- `macOS`
  - `.app`
  - `.dmg`
  - configured through `src-tauri/tauri.macos.conf.json` as `app` and `dmg`
