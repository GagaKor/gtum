...
The macOS CI smoke path uses `npm run tauri:build`, which runs `tauri build --no-bundle`. This validates the native macOS app compile path before release packaging without requiring code signing or notarization.
...
- `ci.yml`
  ...
  - macOS native app build smoke validation
- `release.yml`
