...
- Closed composer provider/model/reasoning/fast controls use compact non-wrapping marks, with exact names/current state in opened lists and exact accessible labels retained. Every popup remains inside the Agent panel and viewport at the default, 260 px, and 240 px Agent widths. Current account labels stay on one line at the default width, longer valid labels wrap without clipping, and the selected row scrolls into view when reopened. Each successfully returned startup connection and every connect, disconnect, reconnect, provider-action error, or capability-read error invalidates the stale provider catalog before a fresh active-provider read. The current slice passes lint, production build, serial Playwright 221/221, and Rust 169 passed / 1 ignored; the ignored prompt-free live catalog smoke passes separately. Non-force `dev` integration remains pending until the active repair plan closes.
