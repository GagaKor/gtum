...
- [Multi-Account Agent Profiles Implementation Plan](./superpowers/plans/2026-07-20-multi-account-agent-profiles.md)
...
- 아키텍처, 런타임 책임, 플랫폼 전략, auth와 contract
  - [기술 설계서 / Technical Design](./technical-design.md)
  - [Multi-Account Agent Profiles Implementation Plan](./superpowers/plans/2026-07-20-multi-account-agent-profiles.md) for the active account registry, exact account leases, isolated CLI roots, platform gates, session migration, and no-fallback contract
...
- [Claude Model-Specific Effort and Fast Mode](./superpowers/plans/2026-07-15-claude-effort-fast-mode.md)
  - Read this for the per-model `executionOptions`, request validation, and exact Claude CLI effort/settings baseline. Its original provider-scoped persistence is extended by the current multi-account plan.
- [Selected Provider and Direct Fast Toggle Implementation Plan](./superpowers/plans/2026-07-16-selected-provider-fast-toggle.md)
...
- [Multi-Account Agent Profiles Implementation Plan](./superpowers/plans/2026-07-20-multi-account-agent-profiles.md)
  - Read this for the active multi-account registry, exact provider/account ownership, isolated Codex and Claude CLI contexts, coordinated session-v2 migration, compact account picker, account Settings flow, and the explicit prohibition on automatic rotation or fallback.
- [Provider-Aware Model Selection Implementation Plan (Historical Baseline, Superseded)](./superpowers/plans/2026-07-15-provider-aware-model-selection.md)
...
- The reserved ambient Claude contract selects an explicit API key, then a strict helper, then an already authenticated user-owned local CLI session. Additional supported Claude profiles are isolated CLI-session contexts only. GTUM performs no Claude.ai OAuth/token capture, and public CLI-session distribution is blocked pending Anthropic approval/contract review.
- The active account-profile contract lets each Agent session manually select an exact Codex or Claude profile. It never rotates accounts for quota, load-balances requests, or falls back to another profile. Each provider has one reserved ambient profile within the limit of 16 total visible-or-tombstoned profiles; generated account IDs and incarnations are immutable, while metadata and credential revisions are tracked separately.
- Additional Codex profiles use isolated `CODEX_HOME` roots with a proven file credential-store configuration. Additional Claude CLI-session profiles use isolated `CLAUDE_CONFIG_DIR` roots on Linux and Windows only; macOS remains ambient-only because Claude credentials are held in Keychain. Setup commands are transient copy-only guidance, and disconnect or Forget never performs provider logout or credential deletion.
