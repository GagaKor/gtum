...
## Current Agent Runtime Contract (2026-07-15)
...
- The frontend owns project/session generations, deduplicated reads and cancels, stale-response rejection, final-log polling, and a bounded right-panel job map. Agent session identifiers, their selected `providerId`, and trimmed provider-keyed `selectedModels` are persisted separately from conversation content so restored request, model, and job ownership remain addressable.
...
- `read_agent_provider_capabilities` is the model-catalog authority. The frontend rejects top-level or per-model provider mismatches, blank IDs/labels, and duplicate IDs before a catalog can affect UI or request state. A selection is stored by `project + Agent session + provider`; it is removed only after a supported catalog definitively omits it, while unavailable or not-yet-loaded capability state preserves it. An ineffective selection is sent as `null` so the runtime default applies.
- Claude capability discovery exposes exactly `default`, `best`, `sonnet`, `opus`, and `haiku` when its CLI is available. `best` leaves account-entitlement resolution to Claude Code. Direct `fable`, exact-version, 1M-context, effort, and fast-mode controls remain deferred until structured entitlement discovery is available, and organization-managed policy remains authoritative.
...
- Provider-aware model evidence is separate and newer: the Claude runtime module passes 44/44, the runtime suggestion service passes Playwright 24/24, and the provider workspace/model UI slice passes Playwright 5/5. The complete post-change gate passes lint, production build with only the existing greater-than-500-KB chunk warning, serial Playwright 210/210, Rust formatting/check, and the full Rust suite 149/149. Fake-child coverage proves an explicit alias adds one separate `--model`, `opus` pair, while invalid IDs fail before spawn and implicit default adds no flag. No live `claude -p` inference was run for this slice.
...
- `ClaudeAdapter`
  ...
  - advertise only the bounded model aliases `default`, `best`, `sonnet`, `opus`, and `haiku`; validate an explicit selection against that table before spawn and append it as one `--model <alias>` pair
...
- Provider model selection is exposed only when `read_agent_provider_capabilities` returns a supported, provider-owned catalog. Codex models come from the local Codex CLI catalog/config path and Claude exposes the bounded aliases `default`, `best`, `sonnet`, `opus`, and `haiku`. The selected ID is persisted by project, Agent session, and provider, then forwarded only while it remains valid for that provider. A definitively stale value is removed and sent as `null`; unavailable or not-yet-loaded capability state does not erase it. Codex forwards an explicit ID as `codex exec --model <id>`, while Claude appends exactly one validated `--model <alias>` pair and leaves implicit default requests flag-free. Attachment controls follow the same capability contract; Codex image attachments selected through the composer are forwarded as `codex exec --image <path>`.
