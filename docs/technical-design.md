...
- `read_agent_provider_capabilities` is the model-catalog authority. The Tauri command is asynchronous and moves provider capability work onto a blocking worker so a five-second CLI probe cannot block the IPC executor. The frontend rejects top-level or per-model provider mismatches, blank IDs/labels, and duplicate IDs before a catalog can affect UI or request state. Renderer capability ownership uses provider-scoped generations reconciled for each successfully returned startup connection and every connect, disconnect, reconnect, provider-action error, or capability-read error. Reconciliation invalidates the old catalog synchronously, schedules a fresh read for the connected active provider, and ignores late success or rejection from an older capability or connection generation. A selection is stored by `project + Agent session + provider`; it is removed only after a supported catalog definitively omits it, while unavailable or not-yet-loaded capability state preserves it. An ineffective selection is sent as `null` so the runtime default applies.
...
- An explicit Claude request performs a fresh bounded catalog read after authentication and before inference spawn. The selected string must exactly equal a returned `value`, remains a separate child argument after `--model`, and is rejected before inference if discovery fails or the value disappeared. `model: null` skips that extra catalog probe and preserves the CLI runtime-default path without a model flag.
...
An implicit runtime default leaves both request shapes unchanged and does not require a second catalog probe. An explicit selection first refreshes the authenticated catalog, requires exact membership in its returned sanitized `value` fields, then appends `--model` and that value as separate child arguments. A `resolvedModel` string, historical version, Fable variant, or extended-context value is not selectable unless the current response returns it as its own `value`; Claude Code account entitlement and organization-managed policy remain authoritative.
