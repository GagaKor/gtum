...
- command execution is user-owned; agent command review and decisions stay in the right panel, and approval decisions must not dispatch commands through the terminal runtime
...
- The right agent workspace owns Codex conversation, command review, and decision recording. `Deny` keeps the refusal in the agent panel. `Allow once` and `Always allow` record approval in the Agent panel without opening terminal tabs, writing into existing terminals, or dispatching through the terminal runtime.
