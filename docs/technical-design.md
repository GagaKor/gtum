...
- Focused provider-aware model evidence is separate and newer: the Claude runtime module passes 44/44, the runtime suggestion service passes Playwright 24/24, the provider workspace/model UI slice passes Playwright 5/5, and the touched slices pass lint, production build, and Rust formatting checks. Fake-child coverage proves an explicit alias adds one separate `--model`, `opus` pair, while invalid IDs fail before spawn and implicit default adds no flag. The complete post-change regression suite remains pending; no live `claude -p` inference was run for this slice.
