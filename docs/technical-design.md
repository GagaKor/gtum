...
## Active Multi-Account Runtime Contract (2026-07-20)

- `src-tauri/src/runtime/auth/mod.rs` owns one versioned v2 registry for Codex and Claude profiles. The reserved ambient IDs are `codex-default` and `claude-default`; generated IDs use `<provider>-profile-<lowercase-base36>` such as `codex-profile-1`, are at most 64 ASCII bytes, never change, and are never reused.
- A provider admits at most 16 visible-or-tombstoned profiles. Exactly one visible profile is default. Reserved ambient profiles cannot be forgotten; forgetting a non-default profile retains its tombstone, incarnation, credential revision, and owned root, so neither capacity nor an old lease can be bypassed by recreating an identifier.
- Every profile carries `provider`, `accountId`, user-authored `alias`, tagged profile kind, immutable `incarnation`, `metadataRevision`, `credentialRevision`, default state, and connection state. Aliases are trimmed 1–64-byte UTF-8 without control characters and are never provider-derived. IPC serializes all three revision/incarnation values as canonical decimal strings so JavaScript never loses `u64` precision.
- Alias/default changes advance metadata only. Connection or execution-context lifecycle changes advance the credential revision. Provider execution and approval authorization use the exact `provider + accountId + incarnation + credentialRevision` lease, never the metadata revision and never a current default lookup.
- v1 provider-only auth state migrates atomically to deterministic ambient profiles before the renderer can list accounts. The migration scrubs pending login, callback, identity, and raw diagnostic data, copies no credential, preserves explicit disconnect semantics, and fails closed on unsupported future versions or invalid ownership. On every later v2 restart, each persisted `Connected` profile becomes `requiresValidation: true` and advances its credential revision before publication; generated profiles remain visible as `Needs verification`, but exact connected-lease work stays blocked until a fresh account-specific Check succeeds.
- Profile creation provisions an app-owned canonical root while holding the registry lock. POSIX directories are `0700` and generated config files are `0600`; Windows requires an owner-only app-data ACL and rejects symlink, junction, or reparse escapes. Every child launch reopens and revalidates the selected root so a path swap cannot redirect credentials.
- Additional Codex profiles are `codex_home` profiles. Their child environment removes ambient Codex/OpenAI credential overrides before setting the exact `CODEX_HOME`. Creation is supported only after a behavioral probe proves the installed CLI accepts and honors `cli_auth_credentials_store = "file"`; validation uses `codex login status` and never reads or parses `auth.json`.
- Additional Claude profiles are `claude_config_dir` CLI-session profiles on Linux and Windows. Their child environment removes API keys, OAuth tokens, helpers, alternate hosts, cloud selectors, and all competing Claude credential sources before setting the exact `CLAUDE_CONFIG_DIR`. macOS additional Claude profiles fail before spawn because Claude stores credentials in Keychain; the ambient `claude-default` profile keeps the existing approved credential precedence.
- On Windows, a captured `CLAUDE_CODE_GIT_BASH_PATH` must remain an absolute canonical regular `.exe`, retain the same identity, and be revalidated before exact reinjection into the scrubbed Claude child environment. It is a bootstrap dependency, not an account credential or fallback route.
- `read_agent_profile_snapshot` provides one atomic all-provider snapshot; `list_agent_profiles` is a provider-filtered visible-profile query. Account lifecycle commands create, rename, set default, check, disconnect, forget, generate setup guidance, and authorize an exact lease. Account diagnostics, capabilities, and suggestions use distinct provider-plus-account commands; legacy provider-only commands fail closed and must not infer an account. Startup serializes provider validation, the exact profile snapshot, and Agent-session hydration so a superseded or failed read cannot commit an empty legacy migration.
- Renderer session v2 stores `selectedAccountIds` per provider and persists model, reasoning, and Fast state by exact account. Attachment catalogs/selections remain account-scoped in memory rather than entering the session directory. Hydration waits for the authoritative profile snapshot. Only a genuinely new session may seed one current default; legacy, missing, disconnected, malformed, forgotten, or unavailable selections remain unassigned or blocked without fallback.
- Capability caches, in-flight requests, action generations, responses, errors, cancellations, suggestion cards, and permission turns use composite account ownership. One immutable send snapshot freezes project, Agent session, provider, account, alias, incarnation, credential revision, model, reasoning, Fast, attachments, and tab context before the first asynchronous boundary.
- Approval uses only `create_authorized_agent_job`. That single backend command holds the auth-store lock while it verifies the exact connected `provider + accountId + incarnation + credentialRevision` lease and creates the isolated Agent job, so Disconnect, Forget, or credential-context refresh cannot commit in the authorization-to-spawn gap. The provider-less `create_agent_job` IPC is not registered, and its TypeScript compatibility methods fail closed whenever a desktop runtime exists. Rename or default metadata changes do not stale the lease. Jobs remain durably addressed by project plus Agent session rather than becoming an authentication ledger.
- Setup guidance returns structured program/environment/arguments and a rendered shell command only on demand. The renderer treats it as transient copy-only data and must not persist, log, execute, or place it in a conversation. Disconnect and Forget modify only GTUM profile state; neither deletes credentials, removes the owned root, invokes provider logout, or promises revocation.
- No account is selected automatically to recover from quota, failure, disconnect, unsupported platform behavior, or stale state. Profile work and all approved Agent work remain isolated from the user-visible center terminal.
...
- The frontend owns project/session generations, deduplicated reads and cancels, stale-response rejection, final-log polling, and a bounded right-panel job map. Agent session identifiers, their selected `providerId`, provider-keyed `selectedAccountIds`, and account-nested `selectedModels`, `selectedReasoningLevels`, and `fastModes` are persisted separately from conversation content so restored request, option, exact-account, and job ownership remain addressable.
- `Codex` is available only after exact-profile local CLI-session validation. A v2 request obtains an exact account lease, then performs validation and optional execution on a blocking worker inside the frozen profile context. Validation results remain publishable only while `provider + accountId + incarnation + credentialRevision` is current, so late results cannot cross disconnect, context change, Forget, or another profile. CLI discovery, login status, and catalog probes have bounded subprocess timeouts.
- `Claude` is exposed as an available, real provider through the same typed frontend auth and suggestion seams. A session can select and persist an exact Claude account independently from every other session; the request includes `agentSessionId + accountId`, and the frontend rejects blank ownership or a response from a different provider/account before rendering.
...
- `read_agent_account_capabilities` is the v2 model-catalog authority. The Tauri command is asynchronous and moves account-context capability work onto a blocking worker so a bounded CLI probe cannot block the IPC executor. The frontend rejects top-level provider/account owner mismatches, cross-provider models, blank IDs/labels, and duplicate IDs before a catalog affects UI or request state. Account-lease generations invalidate older catalogs and ignore stale success or rejection across startup, check, disconnect, Forget, context change, or capability-read error. A selection is stored by project + Agent session + provider + account and cannot be replaced by another account's default. Auth discovery and capability reads remain Agent-owned and never mutate the center terminal. `read_agent_provider_capabilities` remains only for legacy migration compatibility.
...
- The active frontend agent-owned execution seam is `src/shared/api/runtimeAgentJobs.ts`; it wraps project-scoped `create_authorized_agent_job`, `list_agent_jobs`, `read_agent_job_logs`, and `cancel_agent_job` so approved agent commands run as hidden background jobs instead of mutating center terminal tabs. Job creation carries the captured exact account lease and performs authorization plus creation under one backend auth critical section; the provider-less generic creation command is not registered.
...
1. the user selects or creates one exact account profile for the provider in the active Agent session
2. for a new supported profile, the runtime provisions its isolated root and returns transient external CLI-login guidance; it never runs that command or writes it into the center terminal
3. `check_agent_profile` validates only the targeted provider/account context. Codex uses the exact ambient or isolated CLI session; additional Claude profiles use isolated Linux/Windows CLI-session roots, while the ambient Claude profile retains its explicit key/helper/CLI precedence
4. if login is missing or the platform is unsupported, the runtime returns exact actionable guidance and does not fall back to another profile
5. the runtime stores only non-secret profile connection state, and the session retains its own `providerId` plus provider-keyed exact `accountId`
...
- The active frontend auth seam is `src/shared/api/runtimeAgentAuth.ts`; its v2 methods wrap the atomic profile snapshot, profile lifecycle, transient setup guidance, and exact lease authorization commands. Legacy connection methods remain available only for migration compatibility.
- In the desktop runtime path, additional profiles use `createProfile`, `readProfileSetupGuidance`, and `checkProfile`. Login remains an external user-terminal action, and no profile flow invokes a center-terminal command.
- Claude remains `available + real`, but each exact profile owns its own state. Additional CLI-session profiles are supported only on Linux/Windows; the ambient profile retains the current API-key/helper/CLI adapter. Availability still does not prove a connected credential or live inference.
...
- Only the fixed `Fast`, `Balanced`, and `Deep` scheduling presets are absent from the agent panel, settings modal, statusbar, workspace persistence, and v2 account request envelope. Capability-backed Fast is implemented as `fastMode` in the composer, account-scoped session persistence, and `request_agent_account_suggestions` envelope.
- Model selection is exposed only when `read_agent_account_capabilities` returns a supported catalog owned by the exact provider/account. Codex models come from that account's isolated CLI context; Claude models are the sanitized exact `value` fields in that account's authenticated initialization response. The selected ID is persisted by project, Agent session, provider, and account, then forwarded only while valid for that exact owner. Invalid or unavailable state never selects another account. Codex and Claude retain their existing validated model argument mappings inside the frozen account context, and attachments follow the same account-owned capability contract.
...
- Reasoning/Fast persistence is nested by provider and account inside each project Agent session. Temporarily selecting a model that cannot use a saved value makes the effective request `null`/`false` without deleting that exact account's preference. Send constructs one immutable provider/account/lease/model/attachment/reasoning/Fast snapshot before state mutation or any asynchronous boundary.
...
- The active v2 frontend seam is `src/shared/api/runtimeAgentSuggestions.ts`; its account methods wrap `read_agent_account_capabilities`, `read_agent_account_diagnostics`, and `request_agent_account_suggestions`, require `agentSessionId + accountId`, reject provider/account-mismatched responses and malformed/cross-owner catalogs, and normalize either provider into an exact-account reply-only or command-bearing turn. Provider-only methods remain migration-compatible only.
