...
## Active Multi-Account Agent Profiles Policy (2026-07-20)

- Heavy users may register multiple supported Codex and Claude account profiles and manually select the exact profile for each project-owned Agent session. This is an explicit user choice, not quota rotation, automatic failover, load balancing, usage-limit evasion, or billing management.
- Every provider has one deterministic reserved ambient profile: `codex-default` or `claude-default`. Exactly one profile is the provider default, but the reserved ambient profile remains non-forgettable even after another profile becomes default. An existing, missing, disconnected, forgotten, or stale session selection never silently redirects to the default or another account.
- Each provider is bounded to 16 visible-or-tombstoned profiles. Generated account IDs are immutable, never reused, and paired with an immutable incarnation. Metadata and credential revisions advance independently so renaming or changing the default does not invalidate credential work, while disconnect or execution-context changes do.
- Aliases are user-authored, trimmed UTF-8 values of 1–64 bytes with no control characters. They may contain user-supplied PII, so GTUM never derives them from provider output and never substitutes email, organization, subscription, or other account identity metadata.
- Existing provider-only installs migrate to the reserved ambient profiles without copying tokens or credentials. Renderer session migration waits for the authoritative profile snapshot, maps legacy provider-only state only to the matching reserved profile, and leaves unavailable or ambiguous ownership unassigned.
- Every persisted connected profile becomes `Needs verification` on desktop restart and advances its credential revision. Reserved ambient profiles may recover through the existing startup validation; a generated profile remains selected but cannot load capabilities or Send until its exact Check succeeds. No other profile is selected as a recovery fallback.
- Additional Codex profiles use isolated `CODEX_HOME` roots only after the installed CLI proves it honors `cli_auth_credentials_store = "file"`. GTUM does not read, copy, create, or parse `auth.json`; users complete CLI login in their own terminal.
- Additional Claude CLI-session profiles use isolated `CLAUDE_CONFIG_DIR` roots on Linux and Windows. macOS remains ambient-only because Claude credentials are held in Keychain and cannot be isolated by that config root. Unsupported profiles fail before child launch and never fall back to ambient credentials.
- Setup guidance is a transient, copy-only command assembled from structured program, environment, and argument fields. It is not persisted, logged, sent into conversation history, or executed in the center terminal.
- Disconnect changes GTUM's connection state for one exact profile. Forget hides a non-default profile and retains a bounded tombstone and credential root. Neither action logs out the provider, deletes credentials, deletes the root, or promises provider-side revocation.
- Model, reasoning, Fast, and attachment preferences remain keyed by provider/account. Diagnostics, capabilities, requests, successful responses, errors, suggestions, and permission turns carry the exact `provider + accountId + incarnation + credentialRevision` runtime lease and reject stale or mismatched completion. Jobs remain durably owned by project plus Agent session, while the originating turn and approval retain that full lease.
- `Allow once` uses one account-authorized job-create boundary. Exact connected-lease verification and isolated job creation are linearized under the backend auth lock, and the provider-less job-create IPC is unavailable; a concurrent Disconnect, Forget, or credential refresh therefore either waits until creation is owned or blocks the stale approval before any job is created.
- Profile management, external login guidance, requests, approvals, and approved work must never create, focus, write to, or otherwise mutate the user-visible center terminal. Approved commands continue to use the isolated Agent-job surface.
- The active implementation and validation checklist is [`docs/superpowers/plans/2026-07-20-multi-account-agent-profiles.md`](./superpowers/plans/2026-07-20-multi-account-agent-profiles.md). The registry, compact picker, Settings management, restart/approval hardening, race/aging coverage, local native build, complete local regression, and independent reviews are present on the verified candidate. Exact commit, non-force `dev` integration, and runner-native CI evidence remain before the integrated slice is called complete.

## Current Runtime Policy (2026-07-15)

- `Codex` and `Claude` are available, real provider choices. Every Codex profile requires a validated local Codex CLI ChatGPT session. The reserved ambient Claude profile uses an explicit non-empty `ANTHROPIC_API_KEY`, then a valid top-level user `apiKeyHelper`, then an already authenticated session in the installed, user-owned Claude Code CLI; additional supported Claude profiles are isolated CLI-session contexts only. Bedrock, Vertex, Foundry, unknown providers, and source mismatches fail closed.
...
- Provider and account selections belong to each Agent session. The session-v2 directory persists `providerId`, provider-keyed `selectedAccountIds`, and account-nested model/reasoning/Fast preferences, so switching projects, sessions, providers, or accounts restores only that exact owner's prior choice.
- Capability catalogs are runtime-owned per exact account. The frontend accepts a catalog only when its top-level provider/account owner matches the request, every current/available model belongs to that provider, and model IDs/labels are nonblank and unique. Account-lease capability generations invalidate old catalogs across profile lifecycle/context changes; only the exact connected selected profile schedules a read, and late success or failure cannot overwrite another account's state.
...
- Reasoning and Fast preferences are persisted independently by provider and exact account for each project Agent session. An unsupported selected model makes the effective request `reasoningLevel: null` and `fastMode: false` without erasing that account's saved preference. A stale Codex reasoning value may recover only to the supported default inside the same account catalog; stale Claude model-owned effort remains `null`/`Default` because the product must not infer a model effort.
...
- The active Agent header and closed composer provider marks carry an explicit selected/current treatment for Codex and Claude alike. Provider identity styling remains independent and must not imply selection or connection readiness. Provider/account, model, and reasoning remain compact non-wrapping menu controls; exact full names and current state appear in their opened lists while accessible names remain exact. Their popups stay inside the Agent panel and viewport, including 240 px and 260 px Agent widths. Current account aliases remain on one visual line at the default desktop width, while longer valid aliases wrap inside their option so the exact name is never clipped. The model list scrolls the selected row into view when reopened and keeps the exact model ID in metadata and the request payload rather than exposing a second raw-ID row.
- Fast is a capability-gated direct boolean button, not a popup or listbox. One native pointer, Enter, or Space activation inverts the state exactly once and closes any other open composer popup. Its `aria-label` and `title` contain the exact `Fast mode: Enabled` or `Fast mode: Disabled` state, and `aria-pressed` contains the matching boolean. Unsupported models hide the button without erasing the exact account-owned saved preference; account/session ownership and request-time freezing remain unchanged.
- Every v2 provider request captures `projectPath + agentSessionId + providerId + accountId + incarnation + credentialRevision`, and every successful diagnostic, capability, or suggestion response echoes the complete runtime lease. Blank ownership, any lease mismatch, or stale credential completion is rejected before account state, a reply, or a permission card is rendered.
...
Users register and manually select exact agent account profiles through app-managed metadata and external CLI-login guidance rather than by pasting raw API tokens into the UI.
...
- each provider exposes one reserved ambient profile and bounded additional profiles; every Agent session remembers one exact account per provider, and request failure never rotates or falls back to another account
- additional Codex profiles use isolated `CODEX_HOME` roots with a proven file credential store; GTUM validates through the CLI and never reads or copies `auth.json`
- the ambient `Claude` path uses an explicit API key first, then a strict top-level user `apiKeyHelper`, then an already authenticated local Claude Code CLI session; that CLI-session source is local/internal-use infrastructure pending Anthropic approval for third-party distribution
- additional Claude CLI-session profiles use isolated `CLAUDE_CONFIG_DIR` roots on Linux/Windows; macOS keeps only the ambient profile because Claude credentials remain in Keychain
...
- user aliases are treated as user-supplied PII. Setup commands are transient, and Disconnect/Forget do not perform provider logout or credential deletion
...
Current status: deferred for fixed execution policies. The current app does not expose `Fast`, `Balanced`, or `Deep` scheduling modes. It does expose account-backed reasoning and Fast request options when `read_agent_account_capabilities` reports support for the exact selected provider/account/model; Claude controls are model-specific, while Codex retains its provider-level compatibility contract inside that account context. These request options are not scheduling modes. Reintroduce fixed execution policies only after runtime scheduling policies exist.
...
- users should be able to register supported `Codex` and `Claude` profiles through their approved runtime credential contracts, and each Agent session should persist its own selected `providerId` plus exact account selection per provider
...
- the app should expose only model choices owned by the exact active provider/account capability catalog; fixed execution-mode choices remain deferred until scheduling policy support exists
