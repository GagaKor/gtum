...
17. Provider connect, failure, reconnect, disconnect, capability state, and the contained one-line option popups stay in the owning Agent panel and settings row. Closed composer provider/model/reasoning/fast controls use compact non-wrapping icons or marks; exact names and current state render inside the opened lists, while exact accessible names and expanded/selected semantics remain on the triggers/options. Failed/unavailable discovery preserves a stored selection for a later valid read but cannot forward it. None of these paths may create, focus, write to, execute in, or close the user-owned center terminal.
...
- Agent-session provider coverage in `tests/e2e/claude-provider-workspaces.spec.ts`; the earlier provider-aware baseline passed 5/5 focused cases, and the current catalog/picker cases additionally cover exact returned labels and values, right-panel popup containment, one-line rows, selected-bottom-row visibility after reopen, persistence, request ownership, and zero center-terminal calls. Final current-slice counts remain pending the complete gate
