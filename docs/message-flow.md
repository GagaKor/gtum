...
15. Provider connect, failure, reconnect, and disconnect state stays in the owning Agent panel and settings row. None of these paths may create, focus, write to, execute in, or close the user-owned center terminal.
...
5. The runtime path packs the captured provider and Agent session owner, active project metadata, selected file context, recent terminal logs, supported provider options, selected attachments, and the user request into the Flow 4 envelope.
...
8. Agent session-directory metadata is stored separately from conversation content so a restored session keeps the same ID and can address its persisted jobs after reload.
...
- Agent-session provider coverage in `tests/e2e/claude-provider-workspaces.spec.ts`, including 19/19 focused cases for persisted per-session provider choice, cross-project isolation, and reverse-order delayed response ownership
