...
15. Selecting a provider loads its model catalog through `read_agent_provider_capabilities`. The frontend accepts the catalog only when the response owner and every current/available model owner match the requested provider, IDs and labels are nonblank, and model IDs are unique. Claude exposes only `default`, `best`, `sonnet`, `opus`, and `haiku`; `best` delegates account-entitlement resolution to Claude Code. Direct Fable, exact-version, 1M-context, effort, and fast-mode controls remain deferred until structured discovery exists, and organization policy remains authoritative.
16. Provider connect, failure, reconnect, disconnect, and capability state stays in the owning Agent panel and settings row. None of these paths may create, focus, write to, execute in, or close the user-owned center terminal.
...
8. Inside a Claude blocking-worker attempt, the adapter pins one credential/config context for both validation and request. A custom config root is accepted only as an existing canonical directory under the current-user home and is revalidated before child launch. CLI-session mode uses `--safe-mode --setting-sources ""`; API-key/helper mode retains `--bare`, and only helper mode receives the sanitized one-key settings document. The CLI and helper are selected as canonical absolute regular executables; the helper accepts no arguments, whitespace, or shell syntax. The child receives an absolute-only `PATH`, strips competing credential/provider/debug/telemetry/process-wrapper controls, and explicitly disables nonessential traffic plus official-marketplace auto-install. One deadline covers process completion and all stdin/stdout/stderr work; stdout is bounded and stderr is bounded-drained but discarded. The prompt is written to stdin and the process runs in the captured canonical project directory. Model tools, MCP, slash commands, Chrome integration, resume/session flags, and session persistence remain disabled. An explicit model must exactly match one bounded Claude alias and is appended as one separate `--model <alias>` pair; invalid values fail before spawn, and an implicit default adds no model flag. Safe mode excludes user/project customizations, but organization-managed policy and account entitlement remain authoritative. Only a successful JSON envelope containing schema-valid `structured_output` may become a shared suggestion response. Authentication-like errors downgrade only the current Claude lease; non-auth process or schema errors remain request failures. No live inference was run without explicit user approval.
...
- agent auth and suggestion runtime service contract coverage through [`tests/e2e/runtime-agent-auth-service.spec.ts`](../tests/e2e/runtime-agent-auth-service.spec.ts) and [`tests/e2e/runtime-agent-suggestions-service.spec.ts`](../tests/e2e/runtime-agent-suggestions-service.spec.ts); the current suggestion-service slice passes 24/24 focused cases including Claude IPC preservation, required session ownership, top-level/per-model provider mismatch rejection, malformed/duplicate model rejection, and inert browser fallback
...
- Agent-session provider coverage in `tests/e2e/claude-provider-workspaces.spec.ts`; the provider-aware model selection slice passes 5/5 focused cases for distinct provider catalogs, per-project/session/provider persistence and reload, stale cleanup versus unavailable preservation, provider/model request snapshots, accessible picker state, and zero center-terminal calls
